...
bool TodoList::startTask(std::string taskId) {
  auto it = tasks.find([taskId](const Zadatak& z) {return z.id() == taskId;});
  if(it != tasks.end()) {
    it->status() = taskStart;
    return true;
  } else {
...
void TodoList::printWaitingTasks() {
  DoubleLinkedList<Zadatak> temp{tasks.filter([](const Zadatak& z){return z.status() == taskWait;})};
...
void TodoList::printStartedTasks() {
  DoubleLinkedList<Zadatak> temp{tasks.filter([](const Zadatak& z){return z.status() == taskStart;})};
  temp.sort([](const Zadatak& a, const Zadatak& b) { return a.priority() < b.priority(); });
  std::cout << "-------------------------------------------" << std::endl;
  for(auto it = temp.begin(); it != temp.end(); ++it)
    std::cout << *it << std::endl;
...
void TodoList::printFinishedTasks() {
  DoubleLinkedList<Zadatak> temp{tasks.filter([](const Zadatak& z){return z.status() == taskEnd;})};
